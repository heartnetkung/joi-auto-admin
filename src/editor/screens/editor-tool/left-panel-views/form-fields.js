...
const FormFields = (props) => {
  ...
  const onSetDefaultStep = () => {
    ...
    newForm = newForm.map(item => {
      ...
      const isStepValueChanged = rowForm.step && !settingState.steps.find(s => s.value === rowForm.step)
                    ...
                    <Col flex="1">
                      {_.get(settingState?.steps, '[0]') &&
                        <Row>
                          ...
                          <Select defaultValue={!_.isNil(item.step) ? item.step : settingState.steps[0].value}
                            style={{ flex: 1 }}
                            ...
                            {
                              settingState.steps.map((st, si) => <Select.Option key={si.toString()} value={st.value}>{st.value || `step ${si}`}</Select.Option>)
                            }
                          </Select>
                        </Row>
                      }
